...
namespace Radial_BandUnB.FRAP_MDC
{
    public class LaplacianMethod : IDisposable
    {
        ...
        public virtual double GetLaplacian(double[] A_array, double[] A_Lap_array, double[] P_array, double[] P_Lap_array, double[] Sd_array, double[] Sd_Lap_array, double[] Sl_array, double[] Sl_Lap_array)
        {
            return 0d;
        }
    ...
    public class LaplacianMethod2 : LaplacianMethod
    {
        public LaplacianMethod2(double dr, int MaxPoints)
            : base(dr, MaxPoints) { }
        public override double GetLaplacian(double[] A_array, double[] A_Lap_array, double[] P_array, double[] P_Lap_array, double[] Sd_array, double[] Sd_Lap_array, double[] Sl_array, double[] Sl_Lap_array)
        {
            Parallel.ForEach(this.LapPoints, (pr) =>
            {
                ...
                LapFunction(P_array, P_Lap_array,
                    jr, jrm1, jrp1);

                LapFunction(Sd_array, Sd_Lap_array,
                     jr, jrm1, jrp1);

                LapFunction(Sl_array, Sl_Lap_array,
                     jr, jrm1, jrp1);
            });
            return 0d;
        }
