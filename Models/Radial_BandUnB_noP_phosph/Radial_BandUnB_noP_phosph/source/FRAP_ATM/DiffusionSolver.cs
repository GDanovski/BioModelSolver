...
namespace Radial_BandUnB_noP_phosph.FRAP_ATM
{
    public class DiffusionSolver : SolverInterface
    {
        ...
        public double[][] ATM_profile { get; set; }
        ...
        {
            storeProfileIndex = 0;
            ATM_profile = new double[length][];
        }
        private void CopyProfile()
        {
            if (storeProfileIndex >= ATM_profile.Length) return;

            int k = storeProfileIndex;
            int length = MaxPoints * 2;
            var ATM_profile_out = new double[length];

            for (int i = 0, j = length - 1, x = MaxPoints-1; i < MaxPoints; i++, j--, x--)
            {
                ATM_profile_out[i] = ((TheIntegrationMethod.AXl_array[x] + TheIntegrationMethod.PXl_array[x] + TheIntegrationMethod.Bl_array[x]) +
                        (TheIntegrationMethod.Al_T_array[x] - TheIntegrationMethod.AXl_array[x] - TheIntegrationMethod.PXl_array[x] - TheIntegrationMethod.Bl_array[x]));

                ATM_profile_out[j] = ((TheIntegrationMethod.AXl_array[x] + TheIntegrationMethod.PXl_array[x] + TheIntegrationMethod.Bl_array[x]) +
                        (TheIntegrationMethod.Al_T_array[x] - TheIntegrationMethod.AXl_array[x] - TheIntegrationMethod.PXl_array[x] - TheIntegrationMethod.Bl_array[x]));
            }

            ATM_profile[k] = ATM_profile_out;
        ...
        public override double[][] Start(bool printProfile, bool printResults, bool reportProgres, bool reportProgressForm, bool GetImages = false)
        {
            ...
            for (int k = 0; k <= ntimes; k++)
            {
                ...
                {
                    //ManageFRAP
                    if (!SiteParams.FRAP_On && t >= SiteParams.t_FRAP)
                    {
                        SiteParams.FRAP_On = true;
                    ...
                    for (int jr = 0; jr < MaxPoints; jr++)
                    {
                        ...
                        if (R <= SiteParams.S1)
                        {
                            BoundATM_sum += 2 * M_PI * R * dr
                                * (TheIntegrationMethod.GetBoundATM(jr) + TheIntegrationMethod.GetUnboundATM(jr));
