...
namespace Radial_BandUnB_noP_phosph.FRAP_MDC
{
    public class ReactionFunction : IDisposable
    {
        public ReactionFunction() { }
        public virtual void Dispose() { }        
        public virtual double F_AX(double Xm, double AX, double PX, double B, double A_T, double X0_T)
        {
            return 0d;
        }
        ...
        public virtual double F_P(double PX)
        {
            return 0d;
        }
        public virtual double F_B(double AX, double PX, double B,double A_T, double X1_T)
        {
            return 0d;
        }
        public virtual double F_H(double PX, double H, double Y)
        {
            return 0d;
        }
        public virtual double F_Y(double PX, double H, double M, double Y, double S)
        {
            return 0d;
        }
    ...
    public class LogisticReactionFunction : ReactionFunction
    {
        ...
        // X0_T = X0 + [AX] + [PX]
        // X1_T = X1 + B
        public override double F_AX(double Xm, double AX, double PX, double B,
        double A_T, double X0_T)
        {
            // d_dt [AX] = k_on A X - k_off [AX] - k_3 [AX] + k_d [PX]

            double A = A_T - AX - PX - B;
       ...
       public override double F_PX(double AX, double PX)
        ...
        public override double F_B(double AX, double PX, double B,
        ...
        public override double F_H(double PX, double H, double Y)
        {
            return -SiteParams.kfh * PX * H + SiteParams.krh * Y;
           
        }
        public override double F_Y(double PX, double H, double M, double Y, double S)
        {
            return SiteParams.kfh * PX * H - SiteParams.krh * Y + SiteParams.k_off_MDC * M - SiteParams.k_on_MDC * Y * S;
