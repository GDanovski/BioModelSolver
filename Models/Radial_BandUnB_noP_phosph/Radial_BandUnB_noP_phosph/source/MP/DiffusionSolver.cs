...
using ODEModel;
using Radial_BandUnB_noP_phosph.source.common;
...
namespace Radial_BandUnB_noP_phosph.MP_ATM_MDC
{
    public class DiffusionSolver : SolverInterface
    {
        ...
        public double[][] dmg_profile { get; set; }
        public double[][] AX_PX_profile { get; set; }
        public double[][] PX_profile { get; set; }
        public double[][] AX_profile { get; set; }
        public double[][] X0_T_profile { get; set; }
        public double[][] P_profile { get; set; }
        public double[][] A_P_AX_PX_profile { get; set; }
        ...
        public DiffusionSolver(Data data = null)
        {
            ...
            //this.SiteParams.tM_FRAP = data.GetVariable("tM_FRAP").Value;
            //this.SiteParams.R_F_MDC = data.GetVariable("R_F_MDC").Value;
            //this.SiteParams.R_F_ATM = data.GetVariable("R_F_ATM").Value;
        ...
        private void InitProfile(int length)
        {
            ...
            dmg_profile = new double[length][];
            AX_PX_profile = new double[length][];
            PX_profile = new double[length][];
            AX_profile = new double[length][];
            X0_T_profile = new double[length][];
            P_profile = new double[length][];
            A_P_AX_PX_profile = new double[length][];
        }
        private void CopyProfile()
        {
            ...
            var dmg_profile_out = new double[length];
            var AX_PX_profile_out = new double[length];
            var PX_profile_out = new double[length];
            var AX_profile_out = new double[length];
            var X0_T_profile_out = new double[length];
            var P_profile_out = new double[length];
            var A_P_AX_PX_profile_out = new double[length];

            for (int i = 0, j = length - 1, x = MaxPoints-1; i < MaxPoints; i++, j--, x--)
            {
                M_profile_out[i] = TheIntegrationMethod.M_array[x] + TheIntegrationMethod.S_array[x];
                dmg_profile_out[i] = TheIntegrationMethod.X0_profile[x];
                AX_PX_profile_out[i] = TheIntegrationMethod.AX_array[x] + TheIntegrationMethod.PX_array[x];
                PX_profile_out[i] = TheIntegrationMethod.PX_array[x];
                AX_profile_out[i] = TheIntegrationMethod.AX_array[x];
                X0_T_profile_out[i] = TheIntegrationMethod.X0_T_array[x];
                P_profile_out[i] = TheIntegrationMethod.P_array[x];
                A_P_AX_PX_profile_out[i] = TheIntegrationMethod.P_array[x] + TheIntegrationMethod.A_array[x] + TheIntegrationMethod.AX_array[x] + TheIntegrationMethod.PX_array[x];

                M_profile_out[j] = TheIntegrationMethod.M_array[x] + TheIntegrationMethod.S_array[x];
                dmg_profile_out[j] = TheIntegrationMethod.X0_profile[x];
                AX_PX_profile_out[j] = TheIntegrationMethod.AX_array[x] + TheIntegrationMethod.PX_array[x];
                PX_profile_out[j] = TheIntegrationMethod.PX_array[x];
                AX_profile_out[j] = TheIntegrationMethod.AX_array[x];
                X0_T_profile_out[j] = TheIntegrationMethod.X0_T_array[x];
                P_profile_out[j] = TheIntegrationMethod.P_array[x];
                A_P_AX_PX_profile_out[j] = TheIntegrationMethod.P_array[x] + TheIntegrationMethod.A_array[x] + TheIntegrationMethod.AX_array[x] + TheIntegrationMethod.PX_array[x];
            }

            M_profile[k] = M_profile_out;
            dmg_profile[k] = dmg_profile_out;
            AX_PX_profile[k] = AX_PX_profile_out;
            PX_profile[k] = PX_profile_out;
            AX_profile[k] = AX_profile_out;
            X0_T_profile[k] = X0_T_profile_out;
            P_profile[k] = P_profile_out;
            A_P_AX_PX_profile[k] = A_P_AX_PX_profile_out;
                    ...
                    for (int jr = 0; jr < MaxPoints; jr++)
                    {
                        ...
                        if (R <= SiteParams.S1_MDC)
                        {
                            BoundMDC_sum += 2 * M_PI * R * dr
                                 * TheIntegrationMethod.GetBoundMDC(jr);
